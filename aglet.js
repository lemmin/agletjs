...
var Aglet = function (element){
	...
	Aglet.prototype.attachElement = function (element, name, move, init) {
		if (name) {
			element.setAttribute(AG_NAME, name);
		}
	...
	Aglet.prototype.attachComponent = function (comp, name) {
		if (!name) {
			name = comp.name;
		}
		if (!comp.init)
		...
		return this.attachElement(copy, name, true, comp.init);
	}
